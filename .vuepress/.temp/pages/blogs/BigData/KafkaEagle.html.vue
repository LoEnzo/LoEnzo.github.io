...
<p>:::</p>
<!-- more -->
<h2 id="kafka-eagle-整体概览图" tabindex="-1"><a class="header-anchor" href="#kafka-eagle-整体概览图" aria-hidden="true">#</a> Kafka Eagle 整体概览图</h2>
<p><img src="@source/blogs/BigData/images/kafkaEagle/ke01.png" alt="img"></p>
<h2 id="安装启动" tabindex="-1"><a class="header-anchor" href="#安装启动" aria-hidden="true">#</a> 安装启动</h2>
<ul>
<li>
<p><a href="http://download.smartloli.org/" target="_blank" rel="noopener noreferrer">下载安装</a></p>
</li>
<li>
<p><a href="http://www.kafka-eagle.org/articles/docs/quickstart/dashboard.html" target="_blank" rel="noopener noreferrer">官方文档</a></p>
</li>
<li>
<p>配置环境变量</p>
</li>
<li>
<p>修改默认配置文件</p>
<p>::: details 默认配置文件修改  安装目录/conf</p>
...
<li>
<p>mysql安装，启动并创建名为<code v-pre>ke</code>的数据库，导入数据库，新版本的<code v-pre>kafka eagle</code>已经没有<code v-pre>sql文本了</code></p>
<p>::: details  导入sql文本</p>
<blockquote>
<p>use ke</p>
<p>source  sql文本路径，导入sql即可</p>
</blockquote>
...
<p>启动</p>
<blockquote>
<p>ke.sh {start|stop|restart|status|stats|find|gc|jdk|version|sdate}</p>
</blockquote>
</li>
</ul>
...
<ol>
<li>
<p>启动各种报错，可以去安装目录下的log日志查看，主要是两方面：</p>
<p>一方面可能是zookeeper没启动起来，或者配置错误；</p>
<blockquote>
<p>KafkaZKPoolUtils.main - ERROR - Error initializing zookeeper, msg is java.lang.NullPointerException</p>
</blockquote>
<p>另外一方面就是连接mysql报错的各种报错，主要检查mysql配置，root是否允许远程登陆，是否是免密登陆，如果是kafka Eagle使用密码登陆也会报错</p>
<blockquote>
<p>MySqlStoragePlugin.Thread-8 - ERROR - Create mysql connection has error address[127.0.0.1:3306],username[root],password[XXX],msg is Access denied for user 'root'@'localhost'</p>
</blockquote>
</li>
</ol>
</div></template>
